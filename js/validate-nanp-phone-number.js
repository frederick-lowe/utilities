/**
 * isNANPCompliantPhoneNumber tests an abritrarily formatted phone num 
 ...
 * @since 1.0
 *
 * @param string || int num - the phone num to test
 * @returns object:
 *  { 
 *    "status" : 
 ...
 *  }
 */
function /* pure */ isNANPCompliantPhoneNumber(num)
{
  if(!num)
  {
    return {"status":"EMPTY"};
  ...
  /* stringify num, and remove all formatting, plus leading 1 if present */
  num = num.toString().replace(/[^0-9]/g,'').replace(/^1?/,'');
  ...
  */
  ...
  /* num is NANP-compliant; all TNI 0000 - 9999 are valid */
  return {"status":"VALID"};
}